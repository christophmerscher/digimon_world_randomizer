"""Offset tables for individual script commands (chests, recruits, …).

Unlike `blocks.py` (which describes contiguous record arrays), this module
collects the scattered byte offsets where the ROM stores one-off script
instructions — chest spawns, map item spawns, Tokomon gifts, recruitment
trigger writes, special-evolution targets, etc.

Each section names the struct format used to read/write a single record
and the tuple of every offset in the ROM that holds that record.
"""


# ---------------------------------------------------------------------------
# Script binary base offset (start of executable script region)
# ---------------------------------------------------------------------------
...
starterChkDigimonOffset      = (0x14CD1D24, 0x14CD1D44)   # check digimon id when learning tech
...
# ---------------------------------------------------------------------------
# Chest items  (chest spawn command + item id)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Tokomon item gifts  (giveItem command + item id + count)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Tech gifts  (Seadramon + Beetle Land)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Recruitment triggers  (trigger checks + names to substitute)
# Tuple shape: ( trigger-check offsets, name-write offsets, trigger byte, digimon id )
# ---------------------------------------------------------------------------
...
recruitOffsets = (
    ...
    (
        ...
        (),
        210, 0x0A,
    ),  # Seadramon -- does nothing in town and has no Jijimon message
    (
        (
    ...
    (
        ...
        (),
        226, 0x1A,
    ),  # SkullGreymon  -- does nothing except tournament
    (
    ...
    (
        ...
        (),
        254, 0x36,
    ),  # Megadramon -- does nothing and has no Jijimon message
    (
...
# ---------------------------------------------------------------------------
# Special-evolution targets
# Tuple shape: ( offsets, check-value (target id), from-value (current id) )
# ---------------------------------------------------------------------------
