...
All notable changes to this project. The version log was originally
maintained inline in `README.md`; it lives here from the 2026 refactor
onwards.
...
### Internals
    ...
    - Handler god class split into Reader / Writer / Pipeline / per-patch
      and per-randomiser Strategy classes (handler.py: 2,084 to 262 lines).
    - data.py split into focused modules under digimon/rom/.
    - Settings split into a focused subpackage.
    - Models decoupled from handler via NameLookup / RosterLookup Protocols.
    - Test suite expanded from 20 to 69 tests.
    - Behaviour preserved, byte-identical ROM output for the same seed.
